...
When all stories for the milestone are defined (or when the user indicates they're done), offer two options for validation:

**Ask:** "We've defined the stories for this milestone. Before we wrap up, would you like to:

1. **Detailed review** - Let's review each story in detail together
2. **Gap analysis** - Let me spin up a subagent to find gaps and blind spots based on the milestone deliverables
...
If they choose **detailed review**: Walk through each story, summarizing the narrative, persona, and acceptance criteria, and asking if anything is missing or unclear.

If they choose **gap analysis**: Launch a subagent to analyze the stories against the milestone's deliverables in ROADMAP.md for completeness, missing user flows, scope gaps, and unclear acceptance criteria. The subagent reads the documents cold without conversation bias.
