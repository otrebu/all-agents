...
**Use Bash tool ONLY:** `pnpm gh-search`

```bash
pnpm gh-search "your query"
```
...
2.  **Execute:** Run `pnpm gh-search "query"` sequentially.
...
**Required:** `gh` CLI authenticated (`gh auth login`) or `GH_TOKEN`.

## Implementation

Scripts: `docs/knowledge/github/scripts/`
