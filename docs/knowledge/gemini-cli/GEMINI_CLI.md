...
description: Web research via Gemini CLI with Google Search grounding. Generates raw JSON data and a Markdown placeholder. Claude MUST append analysis section after reading raw data.
...
```bash
pnpm gemini-research "your query here" [--mode quick|deep|code]
```
...
```bash
pnpm gemini-research "TypeScript error handling patterns 2025"
```
...
```bash
pnpm gemini-research "React Server Components best practices" --mode deep
```
...
```bash
pnpm gemini-research "Playwright headless browser automation" --mode code
```
