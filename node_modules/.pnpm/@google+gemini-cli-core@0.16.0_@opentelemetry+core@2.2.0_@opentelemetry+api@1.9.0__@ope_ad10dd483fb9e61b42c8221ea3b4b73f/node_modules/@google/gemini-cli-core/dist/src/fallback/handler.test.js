...
describe('handleFallback', () => {
    ...
    describe('Preview Model Fallback Logic', () => {
        ...
        it('should NOT set fallback mode if user chooses "retry_once"', async () => {
            ...
            const result = await handleFallback(mockConfig, PREVIEW_GEMINI_MODEL, AuthType.LOGIN_WITH_GOOGLE, new Error('Capacity'));
            expect(result).toBe(true);
            expect(mockConfig.setPreviewModelBypassMode).toHaveBeenCalledWith(true);
        ...
        it('should pass DEFAULT_GEMINI_MODEL as fallback when Preview Model fails', async () => {
            ...
            expect(mockConfig.fallbackModelHandler).toHaveBeenCalledWith(PREVIEW_GEMINI_MODEL, DEFAULT_GEMINI_MODEL, undefined);
        });
    });
    it('should return null if ModelNotFoundError occurs for a non-preview model', async () => {
        ...
        const result = await handleFallback(mockConfig, DEFAULT_GEMINI_MODEL, // Not preview model
        AUTH_OAUTH, modelNotFoundError);
        expect(result).toBeNull();
        expect(mockHandler).not.toHaveBeenCalled();
    });
