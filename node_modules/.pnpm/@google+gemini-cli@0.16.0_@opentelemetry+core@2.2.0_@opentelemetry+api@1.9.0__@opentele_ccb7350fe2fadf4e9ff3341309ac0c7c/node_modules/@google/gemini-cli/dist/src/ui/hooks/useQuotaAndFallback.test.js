...
import { UserTierId, AuthType, TerminalQuotaError, makeFakeConfig, RetryableQuotaError, PREVIEW_GEMINI_MODEL, ModelNotFoundError, } from '@google/gemini-cli-core';
        ...
        describe('Interactive Fallback', () => {
            ...
            for (const { description, error } of testCases) {
                it(`should handle ${description} correctly`, async () => {
                    ...
                    expect(mockHistoryManager.addItem).not.toHaveBeenCalled();
                    const message = request.message;
                    expect(message).toContain('model-A is currently experiencing high demand. We apologize and appreciate your patience.');
                    // Simulate the user choosing to continue with the fallback model
                    await act(() => {
                        result.current.handleProQuotaChoice('retry_always');
                    });
            ...
            it('should handle ModelNotFoundError correctly', async () => {
                ...
                const message = request.message;
                expect(message).toBe(`It seems like you don't have access to Gemini 3.
...
To disable Gemini 3, disable "Preview features" in /settings.`);
    ...
    describe('handleProQuotaChoice', () => {
        ...
        it('should show a special message when falling back from the preview model', async () => {
            ...
            let promise;
            await act(() => {
                promise = handler(PREVIEW_GEMINI_MODEL, 'gemini-flash', new Error('preview model failed'));
            });
            await act(() => {
                result.current.handleProQuotaChoice('retry_always');
            });
            ...
            const lastCall = mockHistoryManager.addItem.mock.calls[0][0];
            expect(lastCall.type).toBe(MessageType.INFO);
            expect(lastCall.text).toContain(`Switched to fallback model gemini-flash. We will periodically check if ${PREVIEW_GEMINI_MODEL} is available again.`);
        });
    });
});
