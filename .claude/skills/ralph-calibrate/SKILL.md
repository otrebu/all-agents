...
4. Resolve `subtasks.json` path by searching up from the current directory to project root.
5. Build command: `aaa ralph calibrate <check> [--review] [--force]`.
6. Run the command via Bash tool (set workdir to the resolved `subtasks.json` directory when needed).
...
```bash
aaa ralph calibrate <check> [--review] [--force]
```
