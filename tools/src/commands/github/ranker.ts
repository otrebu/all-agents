...
function calculateCodeQualityScore(result: SearchResult): number {
  ...
  // Positive indicators
  if (pathLowercase.includes("/src/") || pathLowercase.includes("/lib/"))
    score += 0.2;
  if (pathLowercase.endsWith(".ts") || pathLowercase.endsWith(".tsx"))
    score += 0.1;
  if (
    pathLowercase.includes("/components/") ||
    pathLowercase.includes("/hooks/")
  )
    score += 0.1;

  // Negative indicators
  if (pathLowercase.includes("node_modules")) score -= 0.5;
  if (pathLowercase.includes("/dist/") || pathLowercase.includes("/build/"))
    score -= 0.3;
...
function getRankedResults(
  ...
  const highStarCount = ranked.filter(
    (r) => r.stars >= MIN_STARS_THRESHOLD,
  ).length;
...
export default getRankedResults;
