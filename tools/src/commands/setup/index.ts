...
async function setupUser(): Promise<void> {
  ...
  // Step 3: Symlink
  const existingTarget = getSymlinkTarget(AAA_SYMLINK)
  if (existingTarget === binPath) {
...
async function setupProject(): Promise<void> {
  ...
  if (!isCliInstalled()) {
    p.log.warn('CLI not installed. Run: aaa setup --user (or bun run dev setup --user)')
  }
...
async function handleClaudeConfigDir(): Promise<void> {
  const {status, current, expected} = getClaudeConfigStatus()
